...
InitialFolder='/Users/bong-iquan/MATLAB/sst1.1_mutant/Slow_swim/GeneralParameters_MedianPerFish_EntierDuration';
 
% Figure out how many trials to analyse
cd(InitialFolder)
list=dir('*DataSetPreEscape_FWDvsRT_thres30*');
...
output = struct( 'NTrial', [], 'Fish_ID', [], 'FishGeno',[], ...
        'mBoutFrequency_pre', [], 'mBoutDuration_pre',[],'mNumberOfOscillations_pre',[],'mBoutDistance_pre',[],'mSpeed_pre',[],'mnBout_pre', [], 'mTBF_pre',[],'mAmplitude_pre',[],...
...
%     end;
    ...
    medianIBI_pre{Fish(i)}=median(IBI_pre{Fish(i)},'omitnan');
    medianBoutFrequency{Fish(i)}=1/medianIBI_pre{Fish(i)};
    ...
    TBF_pre{Fish(i)}=(NumberOfOscillations_pre{Fish(i)})/(BoutDuration_pre{Fish(i)});
...
%         Pre_TBFmedianPerBout{Fish(i)}= 0;
        ...
        for h=1:length(idx_PreEscape{Fish(i)});
            ...
            Pre_AmpMaxPerBout{Fish(i)}(h)=max(abs(Pre_Bend_Amplitude{Fish(i)}{h}));
            %MedianBendPerBout{Fish(i)}(h)=median(abs(Pre_Bend_Amplitude{Fish(i)}{h}));
            %Probleme Calculation from datasetPerBout,don't use'InstantaneousTBF'
            %Pre_TBF{Fish(i)}{h} =[datasetPreEscape(idx_PreEscape{Fish(i)}(h)).InstantaneousTBF];%probleme
    ...
    TBF_post{Fish(i)}=(NumberOfOscillations_post{Fish(i)})/(BoutDuration_post{Fish(i)});
...
%         Post_TBFmedianPerBout{Fish(i)}= 0;
        ...
        for h=1:length(idx_PostEscape{Fish(i)});
            ...
            Post_AmpMaxPerBout{Fish(i)}(h)=max(abs(Post_Bend_Amplitude{Fish(i)}{h}));
...
%     NumberOfOscillations{Fish(i)}=[datasetPerBout(allindex{Fish(i)}).NumberOfOscillations];
    ...
    output(i).mBoutFrequency_pre=medianBoutFrequency{Fish(i)};
    ...
    output(i).mTBF_pre=median(TBF_pre{Fish(i)},'omitnan');
    output(i).mAmplitude_pre=median(Pre_AmpMaxPerBout{Fish(i)},'omitnan');
    ...
    output(i).mTBF_post=median(TBF_post{Fish(i)},'omitnan');
    output(i).mAmplitude_post=median(Post_AmpMaxPerBout{Fish(i)},'omitnan');
...
Amplitude_AllTrials_pre{1,1+Compt_NTrial}=Pre_AmpMaxPerBout;
Amplitude_AllTrials_post{1,1+Compt_NTrial}=Post_AmpMaxPerBout;
...
for l=1:length(Fish_G2);
    ...
    G2_AllMaxiBendAmp{ii}{l}= Pre_AmpMaxPerBout{(Fish_G2(l))};
    ...
    G2medianIBI_pre{ii}(l)=median(IBI_AllTrials_pre{ii}{(Fish_G2(l))});
    G2medianIBI_post{ii}(l)=median(IBI_AllTrials_post{ii}{(Fish_G2(l))});
        % Calculate Bout Frequency
    G2_BoutFreq_pre{ii}(l)= 1/(G2medianIBI_pre{ii}(l));
    G2_BoutFreq_post{ii}(l)=1/(G2medianIBI_post{ii}(l));
...
for l=1:length(Fish_G0)
    ...
    G0_AllMaxiBendAmp{ii}{l}= Pre_AmpMaxPerBout{(Fish_G0(l))};
    ...
    % Calculate median IBI
    G0medianIBI_pre{ii}(l)= median(IBI_AllTrials_pre{ii}{(Fish_G0(l))});
    G0medianIBI_post{ii}(l)=median(IBI_AllTrials_post{ii}{(Fish_G0(l))});
    % Calculate Bout Frequency
    G0_BoutFreq_pre{ii}(l)= 1/(G0medianIBI_pre{ii}(l));
    G0_BoutFreq_post{ii}(l)=1/(G0medianIBI_post{ii}(l));
...
%save('Workspace_4manip_SlowSwim_BoutFreq&MaxAmp_20190811');
...
boxplot_2groupes(G2meanNumOfOsc_pre,G0meanNumOfOsc_post);hold on;
...
% subplot(2,4,5)
% title('Bout Frequency(Hz)');hold on;  
% %boxplot_2groupes(G2medianIBI_pre,G2medianIBI_post); hold on;
% boxplot_2groupes(G2_BoutFreq_pre,G0_BoutFreq_pre); hold on;
% ylim([-2 5]);
% hold off;
% 
% subplot(2,4,6)
...
boxplot_2groupes(G2_AllMaxiBendAmp,G0_AllMaxiBendAmp); hold on;
ylim([0 40]);
hold off;
...
Diff_BoutDuration=EffectPercentage(G2medianBoutDuration_pre,G0medianBoutDuration_pre);
Diff_NumOfOsc=EffectPercentage(G2meanNumOfOsc_pre,G2meanNumOfOsc_post);
Diff_BoutDistance=EffectPercentage(G2medianBoutDistance_pre,G0medianBoutDistance_pre);
Diff_Speed=EffectPercentage(G2medianSpeed_post,G0medianSpeed_post);
Diff_TBF=EffectPercentage(G2medianTBF_pre,G0medianTBF_pre);
Diff_MaxAmplitude=EffectPercentage(G2_AllMaxiBendAmp,G0_AllMaxiBendAmp);
