...
output = struct('Fish', [], 'NumberFish',[], 'Genotypes', [], 'geno_index', [], 'G0', [], 'G1', [], ...
    ...
    'BoutDuration_pre',[], 'BoutDuration_post',[], 'NumberOfOscillations_pre',[], 'NumberOfOscillations_post',[], ...
     ...
     'FishGeno',[],'Fish_G2',[], 'Fish_G1',[],'Fish_G0',[],'G2meanBoutDuration_pre',[]);
...
%Plot Het
% G1groupe_pre= ones(1,numel(cell2mat(G1meanBoutDuration_pre)))*2;
% G1groupe_post= ones(1,numel(cell2mat(G1meanBoutDuration_post)))*6;
% 
% [median_G1BDpre,SEM_G1BDpre]=grpstats(cell2mat(G1meanBoutDuration_pre),G1groupe_pre,{'median','sem'});
% [median_G1BDpost,SEM_G1BDpost]=grpstats(cell2mat(G1meanBoutDuration_post),G1groupe_post,{'median','sem'});
% sz=20;
% scatter(G1groupe_pre, cell2mat(G1meanBoutDuration_pre),sz,'g','filled');hold on;
% scatter(G1groupe_post, cell2mat(G1meanBoutDuration_post),sz,'g','filled');
% jitter1;
% xlim([0.5 8]);
% %ylim([0.05 0.25]);
% hold on;
% errorbar(2, median_G1BDpre, SEM_G1BDpre,'k');hold on;
...
% 
% saveas(h1,['IBIs per 10s.fig'])
% saveas(h1,['IBIs per 10s.png'])
